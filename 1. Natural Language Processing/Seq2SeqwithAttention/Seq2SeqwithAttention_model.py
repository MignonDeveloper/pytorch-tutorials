...
class Encoder(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_dim, num_layers, dropout):
        ...
        self.fc_hidden = nn.Linear(hidden_dim * 2, hidden_dim) # as encoder is bidirectional 
        self.fc_cell = nn.Linear(hidden_dim * 2, hidden_dim) # as encoder is bidirectional 

    def forward(self, x):
        # x_shape: (seq_len, batch)
        embedded_input = self.dropout_layer(self.embedding(x))
        # embedded_input: (seq_len, batch, embedding_size)

        encoder_states, (hidden, cell) = self.encoder(embedded_input)
        # hidden, cell state: (2 * layers, batch, hidden)
        # encoder_states: (seq_len, batch, 2 * hidden)

        # 2(bidirectional) * 2(num_layers) = 4
        # 0: forward, 1: backward, 2: forward, 3: backward
        forward = list(range(0, 2 * self.num_layers, 2))
        backward = list(range(1, 2 * self.num_layers, 2))


        hidden = self.fc_hidden(torch.cat((hidden[forward], hidden[backward]), dim = 2)) # concat each direction hidden state
        cell = self.fc_cell(torch.cat((cell[forward], cell[backward]), dim = 2)) # concat each direction cell state
...
class Decoder(nn.Module):
    def __init__(self, input_size, output_size, embedding_size, hidden_dim, num_layers, dropout):
        ...
        self.dropout_layer = nn.Dropout(self.dropout)
        self.embedding = nn.Embedding(self.input_size, self.embedding_size)
        self.decoder = nn.LSTM(input_size=self.hidden_dim * 2 + self.embedding_size, # encoder bidirectional states + input embedding size
                               hidden_size=self.hidden_dim,
                               num_layers=self.num_layers,
                               dropout=self.dropout)
                            #  batch_first=True)

        self.energy = nn.Linear(hidden_dim * 3, 1) # Hidden state of Encoder(bidirectional) + Hidden state of Decoder
        self.softmax = nn.Softmax(dim=0)
        self.relu = nn.ReLU()
        self.fc = nn.Linear(self.hidden_dim, self.output_size) # for concatenated vector

    def forward(self, x, encoder_states, hidden_state, cell_state):
        '''
        ...
        h_reshaped = hidden_state.repeat(sequence_length, 1, 1)

        energy = self.relu(self.energy(torch.cat((h_reshaped, encoder_states), dim = 2)))
        attention_score = self.softmax(energy)
        # attention_score: (seq_length, batch, 1) / Encoder states: (seq_length, batch, 2 * hidden)
        attention_score = attention_score.permute(1, 2, 0)
        # attention_score: (batch, 1, seq_length)
        encoder_states = encoder_states.permute(1, 0, 2)
        # Encoder states: (batch, seq_length, 2 * hidden)

        # (batch, 1, 2 * hidden) -> (1, batch, 2*hidden)
        context_vector = torch.bmm(attention_score, encoder_states).permute(1, 0, 2)

        # (1, batch, 2*hidden + embedding_size)
        decoder_input = torch.cat((context_vector, embedded_input), dim = 2)

        output, (hidden, cell) = self.decoder(decoder_input, (hidden_state, cell_state))
        # output: (1, batch, hidden_dim) / last hidden & cell state: (2, batch, hidden_dim)

        prediction = self.fc(output)
        # prediction: (1, batch, output_size)
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source, target, teacher_force_ratio=0.5):
        ...
        encoder_states, hidden_state, cell_state = self.encoder(source)

        # Grab start token
        x = target[0] # (1, batch)
        for t in range(1, target_length):
            prediction, hidden_state, cell_state = self.decoder(x, encoder_states, hidden_state, cell_state)
